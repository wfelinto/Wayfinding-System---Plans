...
export default function ProjectPictogramsPage({ params }) {
  ...
  async function load() {
    ...
    const [{ data: projectData }, { data, error }] = await Promise.all([
      ...
      // pictograms became per-project (those have no project_id and
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const { error: insertError } = await supabase
        .from("pictograms")
        .insert({ name, image_path: filePath, project_id: projectId });
  ...
  return (
    <div>
      ...
      <h1 className="text-2xl font-semibold text-ink mt-2 mb-1">Pictograms</h1>
      <p className="text-ink/60 mb-6">
        This project's reusable pictogram library. These show up as thumbnails in the pictogram picker
        next to each message in the editor.
      </p>
