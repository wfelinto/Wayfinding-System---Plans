 ...
 * Crosscheck engine.
 *
 * Given the messages required at a decision point and the library of
 * available sign types (the KOP), decides which sign type fits.
 *
...
/**
 * @param {Array<{id: string, text: string, has_pictogram: boolean, priority: number}>} messages
 ...
 */
export function crosscheckDecisionPoint(messages, signTypes) {
  if (!messages || messages.length === 0) {
    return { status: "conflict", signType: null, reason: "No messages assigned to this decision point yet." };
  }

  const messageCount = messages.length;
  const needsPictogram = messages.some((m) => m.has_pictogram);
  ...
  if (needsPictogram) {
    ...
    if (candidates.length === 0) {
      return {
        status: "conflict",
        signType: null,
        reason: "A message here needs a pictogram, but no remaining sign type supports one.",
      };
    }
  }
