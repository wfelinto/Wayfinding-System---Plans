 ...
 * { text, arrow, pictogram_id } slots) and the library of available sign
 * types (the KOP), decides which sign type fits.
 *
...
/** Always returns exactly ten { text, arrow, pictogram_id } slots, filling in blanks. */
export function normalizeMessageSlots(slots) {
  const base = Array.from({ length: MESSAGE_SLOT_COUNT }, () => ({ text: "", arrow: "", pictogram_id: "" }));
  if (!Array.isArray(slots)) return base;
  return base.map((_, i) => ({
    text: slots[i]?.text || "",
    arrow: slots[i]?.arrow || "",
    pictogram_id: slots[i]?.pictogram_id || "",
  }));
}
