...
/** Loads a file from the dot-images bucket and returns it as a canvas, or null on failure/absence. */
async function loadDotImageCanvas(imagePath) {
  if (!imagePath) return null;
  try {
    const { data } = supabase.storage.from("dot-images").getPublicUrl(imagePath);
...
export async function downloadSignReportPdf(plan, imageUrl, decisionPoints, signTypesById) {
  ...
  for (let index = 0; index < orderedPoints.length; index++) {
    ...
    const photoCanvas = await loadDotImageCanvas(point.image_path);
    const artworkCanvas = await loadDotImageCanvas(point.artwork_path);
