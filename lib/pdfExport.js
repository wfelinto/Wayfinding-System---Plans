...
export async function downloadSignReportPdf(plan, imageUrl, decisionPoints, signTypesById, pictogramsById = {}) {
  ...
  for (let index = 0; index < orderedPoints.length; index++) {
    ...
    // near the right edge — "message - arrow - pictogram". Grouped by
    // side for multi-sided signs, with a "SIDE X" header per side.
    sectionTitle("Messages");
    const design = point.sign_type_id ? signTypesById[point.sign_type_id]?.sign_design : null;
    const sides = sidesForDesign(design);
    const sideGroups = sides
      .map((side) => ({ side, messages: nonEmptyMessagesForSide(point.message_slots, side) }))
      .filter((g) => g.messages.length > 0);
    const showSideHeaders = sides.length > 1;

    const lineHeight = 14;
    const totalContentLines = sideGroups.reduce(
      (sum, g) => sum + (showSideHeaders ? 1 : 0) + g.messages.length,
      0
    );
    const messagesBoxHeight = Math.max(100, (totalContentLines || 1) * lineHeight + 16);
    pdf.setDrawColor(220, 220, 220);
    ...
    if (sideGroups.length === 0) {
      ...
      for (const group of sideGroups) {
        if (showSideHeaders) {
          ...
          pdf.text(`SIDE ${group.side}`, panelX + 8, ty);
