...
/**
 * Draws the plan image plus route lines and decision-point markers onto a
 * canvas at the image's native resolution. Optionally puts a red square
 * around one specific point, for the per-sign report.
 */
function drawPlanToCanvas(img, decisionPoints, routeSegments, { highlightId } = {}) {
  ...
  routeSegments.forEach((seg) => {
    const from = decisionPoints.find((p) => p.id === seg.from_point);
    const to = decisionPoints.find((p) => p.id === seg.to_point);
    if (!from || !to) return;
    ctx.beginPath();
    ctx.moveTo(toPxX(from.x), toPxY(from.y));
    ctx.lineTo(toPxX(to.x), toPxY(to.y));
    ctx.stroke();
  });
...
/** Downloads a single-page PDF of the plan with routes and decision points drawn on it. */
export async function downloadPlanPdf(plan, imageUrl, decisionPoints, routeSegments) {
  const img = await loadImage(imageUrl);
  const canvas = drawPlanToCanvas(img, decisionPoints, routeSegments);
  ...
  pdf.save(`${plan.name} - routes and decision points.pdf`);
}
 ...
 */
export async function downloadSignReportPdf(plan, imageUrl, decisionPoints, routeSegments, signTypesById) {
  const img = await loadImage(imageUrl);
  ...
  orderedPoints.forEach((point, index) => {
    ...
    const canvas = drawPlanToCanvas(img, decisionPoints, routeSegments, { highlightId: point.id });
    const dataUrl = canvas.toDataURL("image/png");
    ...
    const messageLines = (point.messages || "").split("\n").map((l) => l.trim()).filter(Boolean);
    boxedText(messageLines, 80);
