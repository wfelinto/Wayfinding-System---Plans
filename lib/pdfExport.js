 ...
 * that dot highlighted in red, and its code labeled at the top.
 */
export async function downloadDotPlanReportPdf(plan, imageUrl, decisionPoints) {
  ...
  orderedPoints.forEach((point, index) => {
    ...
    const areaW = pageWidth - margin * 2;
 ...
 * message's text, arrow, and pictogram.
 */
async function drawMessageBoard(pdf, x, y, w, h, sideLabel, messages, getPictogramCanvas) {
 ...
 */
export async function downloadMessageSchedulePdf(plan, imageUrl, decisionPoints, signTypesById, pictogramsById = {}) {
  const img = await loadImage(imageUrl);
  ...
  for (let index = 0; index < orderedPoints.length; index++) {
    ...
    for (const rect of boardRects) {
      ...
      await drawMessageBoard(pdf, rect.x, rect.y, rect.w, rect.h, rect.side, messages, getPictogramCanvas);
    }
  }
