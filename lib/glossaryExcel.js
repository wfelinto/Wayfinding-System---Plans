...
/**
 * Triggers a download of the project's actual current glossary — every
 * term, including ones added automatically by typing a new message —
 * in the same column format as the template, so it round-trips cleanly
 * if re-uploaded after editing.
 */
export function downloadGlossaryExport(terms, projectName = "Project") {
  const rows = [
    GLOSSARY_HEADERS,
    ...terms
      .slice()
      .sort((a, b) => (a.external_id ?? 0) - (b.external_id ?? 0))
      .map((t) => [t.external_id, t.term_en, t.term_es || "", t.term_fr || "", t.term_pt || ""]),
  ];
  const worksheet = XLSX.utils.aoa_to_sheet(rows);
  worksheet["!cols"] = [{ wch: 6 }, { wch: 28 }, { wch: 28 }, { wch: 28 }, { wch: 30 }];
  const workbook = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(workbook, worksheet, "Glossary");
  XLSX.writeFile(workbook, `${projectName} - glossary.xlsx`);
}
